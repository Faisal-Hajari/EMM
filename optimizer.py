...
def build_finetune_optimizer(config, model, logger):
    ...
    if config["MODEL_TYPE"] == "swin":
        ...
        num_layers = config["MODEL_VIT_DEPTH"]
        get_layer_func = partial(get_vit_layer, num_layers=num_layers + 2)
    else:
        raise NotImplementedError
    
    scales = list(config["TRAIN_LAYER_DECAY"] ** i for i in reversed(range(num_layers + 2)))
    ...
    parameters = get_finetune_param_groups(
        model, logger, config["TRAIN_BASE_LR"], config["TRAIN_WEIGHT_DECAY"],
